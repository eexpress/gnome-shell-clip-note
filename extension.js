...
const Indicator = GObject.registerClass(class Indicator extends PanelMenu.Button {
	_init() {
		...
		const init_file_array =
			[ '1.web.javascript', '2.live.skill', '3.tech.clip', '4.other' ];
		...
		//~ Creat Icon list
		//~ ---------------------------------------------
		//~ "document-open-symbolic","tools-check-spelling-symbolic",
		//"edit-delete-symbolic"
		const gname = [ 'copy', 'open', 'new', 'refresh' ];
		const gicon = [
			'edit-copy-symbolic', 'folder-open-symbolic', 'document-new-symbolic',
		...
		function add_menu(owner, fname) {
			...
			butt.connect('clicked', (self) => {
				GLib.spawn_command_line_async(
					'xdg-open "' + savepath + '/' + butt.filename + '"');
			});
